...
SoftwareSerial myPort(1, 0); // RX = 1, TX = 0

void setup() {
  // put your setup code here, to run once:
  Serial.begin(9600); // Default port used
myPort.begin(9600); // set HM10 serial at 9600 baud rate
...
void loop() {
 ...
 // digitalWrite(0, HIGH); // Enables Bluetoth module
...
myPort.listen();  // listen the HM10 port
  while (myPort.available() > 0) {   // if HM10 sends something then read
    appData = myPort.read();
